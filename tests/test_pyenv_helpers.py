...
def pyenv_setup(settings):
    ...
    def create_pythons(path):
        ...
        for exe in python_exes(
            [f"{ver.major}", f"{ver.major}{ver.minor}", f"{ver.major}.{ver.minor}"]
        ):
    ...
    cache_versions = settings.get("cache", None)
    if cache_versions:
...
def not_installed_output(ver):
    return (
        f"pyenv specific python requisite didn't meet. "
...
def do_run(*args, **kwargs):
    ...
    result = subprocess.run(
        args, shell=False, stdout=subprocess.PIPE, stderr=subprocess.PIPE, **kwargs
    )
    stderr = (
        str(result.stderr, "utf-8").replace("\r\n", "\n").replace("\r", "\n").strip()
    )
