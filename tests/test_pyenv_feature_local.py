...
def test_local_set_minor_version(pyenv):
    assert pyenv.local(Arch("3.9")) == ("", "")
    assert pyenv.local() == (Arch("3.9"), "")
    assert pyenv('version-name') == (Native("3.9.7"), "")

    assert pyenv.local(Arch("3")) == ("", "")
    assert pyenv.local() == (Arch("3"), "")
    assert pyenv('version-name') == (Native("3.10.9"), "")


@pytest.mark.parametrize('settings', [lambda: {'versions': [Native("3.10.9")]}])
def test_local_set_unknown_version(pyenv):
    assert pyenv.local(Arch("3.9.8")) == (not_installed_output(Arch("3.9.8")), "")
...
def test_local_set_many_versions(local_path, pyenv):
    assert pyenv.local(Native("3.9.7"), Native("3.10.9")) == ("", "")
...
def test_local_set_many_versions_one_not_installed(pyenv):
    assert pyenv.local(Arch("3.9.7"), Arch("3.10.9")) == (not_installed_output(Arch("3.10.9")), "")


@pytest.mark.parametrize('settings', [lambda: {'local_ver': [Native('3.9.7'), Native('3.10.9')]}])
