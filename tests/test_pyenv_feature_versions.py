...
@pytest.mark.parametrize('settings', [lambda: {'versions': [Native('3.8.4'), Native('3.8.5')]}])
...
@pytest.mark.parametrize('settings', [lambda: {
        'versions': [Native("3.8.5"), Native("3.8.7"), Native("3.9.1")],
        'global_ver': Native("3.8.7")
    }])
def test_list_current_global_version(pyenv_path, pyenv):
    assert pyenv.versions() == (
        (
            f"  {Native('3.8.5')}\n"
            f"* {Native('3.8.7')} (set by {pyenv_path}{S}version)\n"
            f"  {Native('3.9.1')}"
        ),
        ""
    )
...
def test_list_current_local_version(local_path, pyenv):
    assert pyenv.versions() == (
        (
            f"* {Native('3.8.5')} (set by {local_path}{S}.python-version)\n"
            f"  {Native('3.8.7')}\n"
            f"  {Native('3.9.1')}"
        ),
        ""
...
@pytest.mark.parametrize('settings', [lambda: {
        ...
        'local_ver': [Native('3.8.5'), Native('3.8.7')]
    }])
def test_list_current_local_many_versions(local_path, pyenv):
    assert pyenv.versions() == (
        (
            f"* {Native('3.8.5')} (set by {local_path}{S}.python-version)\n"
            f"* {Native('3.8.7')} (set by {local_path}{S}.python-version)\n"
            f"  {Native('3.9.1')}"
        ),
        ""
    )
